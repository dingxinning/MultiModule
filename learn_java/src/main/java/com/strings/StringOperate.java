...
public class StringOperate {
    ...
    public static void method_get() {
        ...
        //根据字符获取索引
        sop("从开始位置索引 a 出现的位置为：" + str.indexOf('a'));//如果没有找到返回-1
    ...
    public static void method_sub() {
        String s = "123456789";
        sop("原字符串为：" + s);
        String s1 = s.substring(3); //从指定位置到结尾。如果角标不存在，则会出现字符串角标越界异常。
        sop("获取的子串s1为：" + s1);
        String s2 = s.substring(4, 7); //包含头，不包含尾。 获取整个字符串：s.substring(0,s.length());
        sop("获取的子串s2为：" + s2);
    }
