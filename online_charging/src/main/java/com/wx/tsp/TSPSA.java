 ...
 * TSP问题：参考 greedy
 *
 * 模拟退火法是由 Metropolis 于1953 年提出的，是一种基于热力学原理的随机搜索算法，是局部搜索算法的拓展。
 * 它与局部搜索算法的不同之处在于：它以一定的概率选择邻域中目标函数值差的状态。
 */
public class TSPSA {
     ...
     * @throws IOException
     */
    public void init(List<Point> points) throws IOException {
        // 读取数据
        int[] x;
        int[] y;
        ...
        for (int i = 0; i < cityNum; i++) {
            x[i] = points.get(i).getX();// x坐标
            y[i] = points.get(i).getY();// y坐标
        }
        ...
        for (int i = 0; i < cityNum - 1; i++) {
            ...
            for (int j = i + 1; j < cityNum; j++) {
                double rij = Math.sqrt((x[i] - x[j])*(x[i] - x[j]) + (y[i] - y[j])*(y[i] - y[j]))/2;
                // 四舍五入，取整
    ...
    // 复制编码体，复制编码Gha到Ghb
    private void copyGh(int[] Gha, int[] Ghb) {
        for (int i = 0; i < cityNum; i++) {
            Ghb[i] = Gha[i];
        }
    }
    ...
    // 邻域交换，得到邻居
    private void Linju(int[] Gh, int[] tempGh) {
        int i, temp;
        int ran1, ran2;
    ...
    public void solve() {
        ...
        while (k < T) {
            n = 0;
            while (n < N) {
//                for (int mm = 0; mm < cityNum; mm++) {
//                    System.out.print(Ghh[mm]);
//                }
//                System.out.print("    ");
//                for (int mm = 0; mm < cityNum; mm++) {
//                    System.out.print(tempGhh[mm]);
//                }
//                System.out.println();
        ...
        System.out.println("\n最佳长度出现代数：" + bestT);
        System.out.println("最佳长度：" + bestEvaluation);
        System.out.println("最佳路径：");
        StringBuilder string=new StringBuilder();
        for (int i = 0; i < cityNum; i++) {
            if (i != cityNum - 1) {
                string.append(bestGh[i] + "->");
            }else {
                string.append(bestGh[i]);
            }
        }
//        string.append(bestGh[0]);
